...
(function (p5) {
    ...
    (function (tween) {
        class Tween {
            constructor(objStart, deleteOnFinish) {
                this.motions = [];
                this.keyChanges = [];
                this.active = false;
                this.isLoop = false;
                ...
                this.updateBinds = {};
                this.onFinishedPromises = [];
                this.deleteOnFinish = deleteOnFinish;
            }
            resetToStart() {
                for (let index in this.start) {
                    const [obj, key] = StringToObjectAndKey(this.obj, index);
                    obj[key] = this.start[index];
                }
                this.motionStart = this.createStartObject(this.start);
            }
            createStartObject(objStart) {
                const changes = {};
                this.keyChanges.forEach(key => {
                    changes[key] = StringToPath(objStart, key);
            ...
            startTween(Yield = false) {
                ...
                if(Yield){
                    let resolve;
                    const onFinishedPromise = new Promise((res) => {resolve = res});

                    this.onFinishedPromises.push(resolve);
                    return onFinishedPromise;
                }else{
                    return this;
                }
            }
            ...
            bindToUpdate(name, callback){
                this.updateBinds[name] = callback;
                return this;
            }
            unbindFromUpdate(name){
                delete this.updateBinds[name];
                return this;
            }
            update(deltaTime) {
                ...
                if (motion.leftTime >= motion.duration) {
                    ...
                    if (this.currentMotionIndex >= this.motions.length) {
                        if (this.isLoop) {
                            ...
                            //Calling all the resolve methods for every promise that is awaiting the tween to be completed
                            for(let index = this.onFinishedPromises.length - 1; index >= 0; index--){
                                const resolve = this.onFinishedPromises[index];

                                resolve();
                                this.onFinishedPromises.splice(index, 1);
                            }
                ...
                if (!motion.actions)
                    return;
                for (const index in motion.actions) {
                    let action = motion.actions[index];
                    if (action.key && !isNaN(action.target)) {
                        const progress = Math.min(motion.leftTime / motion.duration, 1.0);
                        
                        const [obj, key] = StringToObjectAndKey(this.obj, action.key);
                        obj[key] = this.interpolation(this.motionStart[action.key], action.target, progress, motion.easing);

                        //Calling all the callbacks that are binded to the update method
                        for(const callbackName in this.updateBinds){
                            const callback = this.updateBinds[callbackName];
...
(function (p5) {
    ...
    (function (tween_1) {
        class TweenManager {
            ...
            addTween(object, name, deleteOnFinish = true) {
                const tweenName = name || 'tween' + this.tweens.length;
                const tween = new tween_1.Tween(object, deleteOnFinish);
            ...
            update(deltaTime) {
                ...
                for (const index in tweens) {
                    const tweenItem = tweens[index];
                    const tween = tweenItem.tween;

                    tween.update(deltaTime);
                    if(!tween.active && tween.deleteOnFinish){
                        delete tweenItem.tween;
                        delete tweens[index];
                    }
                }
            }
...
const TweenManager = p5.tween.manager;
